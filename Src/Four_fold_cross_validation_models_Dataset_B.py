...
def train_model(train_dataloader, validation_dataloader, model, criterion, 
	...
	for epoch in range(num_epochs):
		...
		with torch.no_grad():
			...
			train_loss = 0
			val_loss = 0
		for data in (tqdm((train_dataloader), desc=f'Epoch {epoch + 1}/ {num_epochs} - Training')):

			_,images, masks, img_name_b, mask_name_b = data
...
for fold_id in range(k_fold):
	...
	dataset = SegmentationDataset(data_dir='../Dataset/BUS-B/',transform_1 = transform, transform_2 = transform_seg)
	
	
	train, val = get_train_val_dataset_fold(dataset, fold_id)
	...
	train_acc = train_model(train_dataloader, val_dataloader, model, criterion, 
